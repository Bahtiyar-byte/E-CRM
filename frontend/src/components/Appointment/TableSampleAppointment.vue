...
/* Delete item */
const deleteItem = () => {
  appointmentStore.deleteItem(currentId.value)
}

const items = computed(() => appointmentStore.data)
...
<template>
  ...
  <table>
    <thead>
      ...
      <th
        :class="['sortable uppercase text-sm font-normal text-pavitra-600', props.sortTitle === 'address' && props.sortDirection]"
        @click="sort('address')"
      >Address</th>
    ...
    <tbody>
      <tr
        ...
        :key="client.id"
        @click="$router.push(`/appointment/${client.id}/edit`)"
      >
        <TableCheckboxCell
          v-if="checkable"
