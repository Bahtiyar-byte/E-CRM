...
import TableSampleAppointment from '@/components/Appointment/TableSampleAppointment.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Address', title: 'address'},

          {label: 'Contact', title: 'contactId'},{label: 'Assigned User ', title: 'assignedUserId'},];
...
const getAppointmentCSV = async () => {
  const response = await axios({url: '/appointment?filetype=csv', method: 'GET',responseType: 'blob'});
  const type = response.headers['content-type']
  const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
  const link = document.createElement('a')
  link.href = window.URL.createObjectURL(blob)
  link.download = 'appointmentCSV.csv'
  link.click()
}
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('appointment/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    appointmentStore.fetch(null, query.value);
...
<CardBoxModal
  ...
  <SectionHeroBar>Appointment</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for appointment</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleAppointment checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
