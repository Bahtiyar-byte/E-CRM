...
import TableSampleNotes from '@/components/Notes/TableSampleNotes.vue'
...
const filters = ref([]);
const filtersList = [{label: 'Title', title: 'title'},{label: 'Description', title: 'description'},
...
const parseCsv = async () => {
  try {
    ...
    const result = await axios.post('notes/bulk-import', data, {
      headers: {
...
const onConfirmCsv = async () => {
  if (csvFile.value) {
    ...
    notesStore.fetch(null, query.value);
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Notes'
    : 'http://localhost:8080/api-docs/#/Notes';
});
...
<CardBoxModal
  ...
  <SectionHeroBar>Notes</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for notes</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleNotes checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>
