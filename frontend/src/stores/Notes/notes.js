...
export const useNotesStore = defineStore('notes', {
  ...
  actions: {
    async fetch(id = '', query) {
      this.startLoading()
      try {
        const result = await axios.get(`notes${query || (id ? `/${id}` : '')}`);
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/notes', { data: payload });
        this.showNotification('Notes has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/notes/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Notes has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
